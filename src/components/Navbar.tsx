...
import { citySuggestionInfo, getCurrentLocation } from "@/api/weatherData";
...
export default function Navbar({ location }: Props) {
  ...
  const [showSuggestions, setShowSuggestions] = useState(false);
	...
	const { setPlace, setLoadingCity } = useAppState()

	async function handleInputChange(value: string) {
    setCity(value);
    if (value.length >= 3) {
      try {
        const response = await citySuggestionInfo(value);
